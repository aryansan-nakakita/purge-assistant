...
class EnhancedRAGSystem:
    ...
    def _generate_response(self, query: str, relevant_chunks: List[Dict[str, Any]], 
        ...
        INSTRUCTIONS:
        1. Answer the user's query based ONLY on the provided information.
    ...
    def process_documents(self, force_reprocess: bool = False) -> bool:
        ...
        # Check if data paths exist and contain files
        found_files = False
        all_files = []
        ...
        for data_path in config.data_paths:
            ...
            if not files:
                logger.warning(f"No files found in path: {data_path}")
                continue
            
            logger.info(f"Found {len(files)} files in path: {data_path}")
            all_files.extend(files)
            found_files = True
        
        if not found_files:
            logger.error("No files found in any of the specified paths")
            return False
...
def show_main_ui(user_info):
    ...
    with col1:
        ...
        st.markdown(f"""
        <div class="header">
            <h1>🚢 Nakakita Air Purge Level Gauge Assistant</h1>
        ...
        if st.session_state.language == "Japanese":
            nav_labels = ["チャット", "ドキュメントアップロード", "履歴"]
        else:
            nav_labels = ["Chat", "Upload Documents", "History"]
    ...
    with col2:
        ...
        if rag_system.initialized:
            ...
            st.caption(f"Vector store contains {len(rag_system.vector_store.metadata)} chunks")
        else:
        ...
        if st.button(process_docs_text, type="primary", key="process_docs_btn"):
            with st.spinner("Processing documents..."):
                success = rag_system.process_documents(force_reprocess=True)
                if success:
                    st.session_state.processing_stats = rag_system.document_processor.get_processing_stats()
                    st.session_state.success_message = "Documents processed successfully!"
                    st.rerun()
                else:
                    st.session_state.error_message = "Error processing documents"
                    st.rerun()
        ...
        if progress['status'] != 'Idle':
            ...
            <div class="progress-container">
                <div class="progress-bar" style="width: {(progress['current'] / max(1, progress['total'])) * 100}%">
            ...
            if progress['errors']:
                st.error(f"Errors: {len(progress['errors'])}")
                with st.expander("Show errors"):
                    for error in progress['errors']:
                        st.text(error)
        
        # Show relevant documents for the last query
        if st.session_state.messages and st.session_state.messages[-1]["role"] == "assistant":
            ...
            if "relevant_chunks" in last_message and last_message["relevant_chunks"]:
                ...
                for chunk in last_message["relevant_chunks"][:3]:
                    content_type = chunk.get("content_type", "text")
                        ...
                        try:
                            with open(doc_path, "rb") as file:
                                st.download_button(
                                    ...
                                    key=f"download_{chunk['filename']}_{chunk['chunk_index']}"
                                )
                        except Exception as e:
        ...
        if rag_system.initialized:
            ...
            with st.expander("Document Processing", expanded=False):
                ...
                # Visualize document processing stats
                doc_fig = rag_system.document_processor.visualize_processing_stats()
            ...
            with st.expander("Document Categorization", expanded=False):
                ...
                # Visualize categorization stats
                cat_fig = rag_system.categorizer.visualize_categorization_stats()
                st.plotly_chart(cat_fig, use_container_width=True)
            
            # Chunking Statistics
            with st.expander("Text Chunking", expanded=False):
                chunk_stats = rag_system.chunker.get_chunking_stats()
                st.json(chunk_stats)
                
                # Visualize chunking stats
                chunk_fig = rag_system.chunker.visualize_chunking_stats()
                st.plotly_chart(chunk_fig, use_container_width=True)
            
            # Embedding Statistics
            with st.expander("Embedding Generation", expanded=False):
                embed_stats = rag_system.embedding_generator.get_embedding_stats()
                st.json(embed_stats)
                
                # Visualize embedding stats
                embed_fig = rag_system.embedding_generator.visualize_embedding_stats()
                st.plotly_chart(embed_fig, use_container_width=True)
            
            # Vector Store Statistics
            with st.expander("Vector Store", expanded=False):
                vs_stats = rag_system.vector_store.get_search_stats()
                st.json(vs_stats)
                
                # Visualize vector store stats
                vs_fig = rag_system.vector_store.visualize_search_stats()
...
def show_chat_page():
    ...
    with st.form("chat-form", clear_on_submit=True):
        ...
        if submitted and user_input:
            # If system is in Japanese, translate input to English for processing
            if st.session_state.language == "Japanese":
                user_input_en = translate_text(user_input, "English", st.session_state.translator)
            else:
            ...
            st.session_state.messages.append({
                "role": "user",
                "content": user_input,  # Store original language
                "timestamp": timestamp
            })
    ...
    if st.session_state.typing_indicator:
       ...
       if st.session_state.messages and st.session_state.messages[-1]["role"] == "user":
           ...
           # Process the query (always process in English for RAG)
           if st.session_state.language == "Japanese":
               query_to_process = translate_text(last_user_message, "English", st.session_state.translator)
           else:
               query_to_process = last_user_message
        ...
        for i, item in enumerate(reversed(history)):
            with st.expander(f"Query: {item['query']} - {item['created_at']}"):
                ...
                if item.get("relevant_chunks"):
                    ...
                    for chunk in item["relevant_chunks"][:3]:
                        content_type = chunk.get("content_type", "text")
                        content_label = "Table" if content_type == "table" else "Text"
                            ...
                            try:
                                with open(doc_path, "rb") as file:
                                    st.download_button(
                                        ...
                                        file_name=chunk['filename'],
                                        mime="application/octet-stream",
                                        key=f"hist_download_{item['id']}_{chunk['chunk_index']}"
                                    )
                            except Exception as e:
                                st.error(f"Could not load document: {str(e)}")
